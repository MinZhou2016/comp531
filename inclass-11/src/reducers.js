...
const Reducer = (state =  {
	...
	switch(action.type) {
		case 'ADD_TODO':
			// IMPLEMENT ME
			return {nextId: state.nextId + 1, todoItems:[...state.todoItems,{id: state.nextId, text:action.text, done:false}]};
		case 'REMOVE_TODO':
			// IMPLEMENT ME
			return Object.assign({}, state, {todoItems:state.todoItems.filter((element) => element.id != action.id)});
		case 'TOGGLE_TODO':
			// IMPLEMENT ME
			return Object.assign({},state, {todoItems: state.todoItems.map( element =>{
