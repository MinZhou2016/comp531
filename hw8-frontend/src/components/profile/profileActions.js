import Action, { updateSuccess, updateError, resource,navToProfile } from '../../actions';
    ...
    return (dispatch) => {
        if (value) {
            ...
            resource('PUT', field, payload).then((response) => {
                ...
                if (field == 'password')
                    dispatch(updateError('successfully update the password, But will not change'))
                else
                    dispatch(action)
            })
        }
...
export function linkAccount({regUsername, regPassword}) {
    return (dispatch) => {
...
export function linkFacebook() {
    return (dispatch) => {
        window.top.location = 'https://final-social-website.herokuapp.com/link/facebook'
...
export function unlinkAccount(company) {
    return (dispatch) => {
        resource('POST', 'unlink', {company: company})
        .then((response) => {
